...
const ContactUsModal: React.FC<ContactUsModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('https://empress-hill-tool-backend.vercel.app/api/main-website/submit-form', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50 p-4">
            <div className="bg-[#725054] rounded-lg p-6 md:p-8 w-full max-w-md mx-auto relative">
                {/* Close button */}
                <button onClick={onClose} className="absolute top-4 right-4 text-[#E9DEDC] hover:text-white text-2xl font-bold cursor-pointer">
                    ×
                </button>

                {/* Header */}
                <div className="text-center mb-6">
                    <h2 className="text-2xl md:text-3xl font-bold text-[#E9DEDC] mb-2">Download Brochure</h2>
                    <p className="text-[#E9DEDC] text-sm md:text-base">Please enter the details below to download the brochure!</p>
                </div>
                ...
                {submissionState.success && (
                    <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                        <p className="text-sm">Form submitted successfully! Your brochure download will start shortly.</p>
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        {validationErrors.firstName && <p className="text-red-500 text-xs ml-12">{validationErrors.firstName}</p>}
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        {validationErrors.phone && <p className="text-red-500 text-xs ml-12">{validationErrors.phone}</p>}
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        {validationErrors.email && <p className="text-red-500 text-xs ml-12">{validationErrors.email}</p>}
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                        ...
                        <label className="text-[#E9DEDC] text-[10px]  leading-relaxed">
                            By submitting this form you are agreeing to be contacted regarding any special offer or promotion we feel you might be interested in. You agree to our{' '}
                            <Link href="/privacy-policy" className="text-[#BD314C] hover:underline underline" target="_blank">
                                Privacy Policy
                            </Link>
                            .
                        </label>
