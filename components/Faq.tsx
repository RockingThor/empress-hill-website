...
const faqItems: FaqItem[] = [
    {
        ...
        description: ['Vishal Mega Mart', 'Reliance Smart Bazar', 'Zudio', 'Max', 'Pantaloons'],
    },
    {
    ...
    return (
        <div className="w-full space-y-3 lg:space-y-4">
            {faqItems.map((item, idx) => (
              ...
              ${activeFaq === idx ? 'max-h-32 lg:max-h-40 py-3 lg:py-4 px-3 lg:px-4 opacity-100' : 'max-h-0 py-0 px-3 lg:px-4 opacity-0'}
            `}
                        style={{
                        ...
                        <div className="text-gray-700 text-xs lg:text-sm font-medium tracking-wide flex flex-col gap-1">
                            {Array.isArray(item.description)
                                ? item.description.map((dest, index) => (
                                      <div key={index} className="flex items-center gap-2 text-[#725054]">
